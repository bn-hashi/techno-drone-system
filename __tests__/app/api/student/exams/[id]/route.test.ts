...
import { GET } from "@/app/api/student/exams/[id]/route";
...
const makeReq = () => new Request("http://localhost/api/student/exams/exam-1");

describe("GET /api/student/exams/[id]", () => {
  it("test_GET_unauthenticated_returns_401", async () => {
    vi.mocked(getServerSession).mockResolvedValue(null);

    const response = await GET(makeReq(), makeRequest("exam-1"));
  ...
  it("test_GET_not_found_returns_404", async () => {
    ...
    const response = await GET(makeReq(), makeRequest("exam-x"));
  ...
  it("test_GET_owned_returns_200", async () => {
    ...
    mockGetExam.mockResolvedValue({
      exam: { id: "exam-1", userId: "user-1", status: ExamStatus.PASSED },
      answers: [],
    });
