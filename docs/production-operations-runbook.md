...
1. `/flight/dips-permissions` にアクセスし、「許可・承認情報を取得」ボタンが表示されることを
   ...
   意図しないアクセスを避けるための設計)
2. ボタンを1回クリックし、DIPS ログイン画面 (realm `req`) へ遷移することを確認する
   (未連携の場合。連携済みなら遷移せずそのまま結果が表示される)
3. ログイン後、許可・承認情報の一覧 (許可番号・受付番号・許可期間・飛行場所・機体情報) が
   表示されることを確認する。許可期間は `YYYY/M/D` 形式に整形されて表示される (DIPS が
   ISO 形式を返しても生文字列のまま出ないこと)
...
| 症状 | 想定原因 |
|---|---|
| ログイン画面に飛ばない / 404 | realm `req` の認証ベース URL 設定誤り |
...
| 502 (DIPS連携でエラー) | pm2 ログの `droppedEntries`/`issuePaths` (キー名のみ) を確認し、`lib/dips/permissionsSchema.ts` の想定キーとズレていないか突合する。1件も入っていないのに全滅している場合はレスポンス構造自体の変更を疑う |
| ログイン後もログイン誘導が繰り返される (無限ループ) | realm のずれが疑わしい。`DipsAuthRequiredError` が返す realm と画面が使う realm (`req`) が一致しているか確認する |
| 403 / 接続拒否 | IP 制限。本番サーバーから実行しているか確認する |
