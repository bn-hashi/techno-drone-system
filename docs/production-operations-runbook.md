...
### 1-10. 飛行禁止エリア情報取得 (5-5) の疎通確認

**前提**: 1-9 と同じく本番サーバー上でのみ実施可能。realm は `fpl` (機体情報一覧取得の
`utm`・許可・承認系の `req` とは別)。URL `https://<本番ドメイン>/flight/dips-flight-prohibited-areas`
へ直接アクセスする。実装の詳細は `lib/dips/flightProhibitedAreaSchema.ts` のファイル先頭
コメントを参照。
...
1. `/flight/dips-flight-prohibited-areas` にアクセスし、検索範囲 (既定値: 東京駅周辺・半径
   1000m) とエリア種別 (既定でレッドゾーン・イエローゾーンが選択済み) が表示されることを
   確認する
2. 「飛行禁止エリアを検索」ボタンをクリックする。1-9 と同じ「リンク表示 → リンクをクリック」の
   2段階でログイン誘導される (未連携時)
3. 検索結果が0件でもエラーにならず「該当する飛行禁止エリアがありません」と表示されることを
   確認する。**検証環境の飛行禁止エリアは11月7日時点の状態のため、一部エリアのみ登録**
   (設定通知書シート2 E37)。0件表示自体は正常な結果であり、必ずしも不具合ではない
4. `pm2 logs techno-drone --lines 50` を確認し、DIPS 由来のエラーがないことを確認する
   (このレスポンスは個人情報を含まないため、`droppedEntries` のログ内容自体に注意すべき
   PII は無い)

**失敗時の切り分け**: 1-9 の表と同じ切り分けが適用できる (realm は `fpl` に読み替える)。

### 1-11. 飛行計画情報取得 (5-4) の疎通確認

**前提**: realm は `fpl`。URL `https://<本番ドメイン>/flight/dips-flight-plans` へ直接
アクセスする。

**⚠️ 最重要の前提条件**: **検証環境へのサンプルデータは未投入のため、この確認の前に必ず
1-12 (飛行計画通報受付, 5-6) を先に実施してデータを投入すること** (設定通知書「検証環境
での確認ポイント」D36/E36)。5-6 未実施のまま検索すると (「自分の飛行計画のみ」を
チェックしても) 0件のまま返り、実装の正誤を判断できない。

**確認手順**:

1. 事前に 5-6 (飛行計画通報受付) を1回以上実施し、飛行計画IDが発行されていることを確認する
2. `/flight/dips-flight-plans` にアクセスし、「自分の飛行計画のみ検索する」にチェックを入れて
   検索する。1-9/1-10 と同じ「リンク表示 → リンクをクリック」の2段階でログイン誘導される
3. 5-6 で通報した飛行計画が一覧に表示されることを確認する (通報した申請者IDと、飛行計画
   情報取得の申請者IDが同じ `USR063031` であること。設定通知書シート2 D19/E19 に「検証用に
   参照と更新でアカウントを別に払い出しているが、いずれの申請者IDを使用しても問題無い」と
   記載あり)
4. チェックを外して (全ユーザー検索) 同じ範囲を検索し、他事業者の飛行計画が混在していても
   エラーにならないことを確認する。この場合、機体・操縦者等の詳細情報が表示されない
   飛行計画が混在するのは仕様どおり (○/●の区別。`lib/dips/types.ts` の `DipsFlightPlanInfo`
   コメント参照)

**失敗時の切り分け**: 1-9 の表と同じ切り分けが適用できる (realm は `fpl` に読み替える)。
0件が返る場合はまず「1-12 を先に実施したか」を確認すること。

### 1-12. 許可・承認申請受付 (5-3) の疎通確認

**前提**: realm は `req` (許可・承認情報取得 (1-9) と同じ Client ID)。URL
`https://<本番ドメイン>/flight/dips-permission-apply` へ直接アクセスする。

**⚠️ 検証環境の他事業者共用データベースへ実際に申請データを登録する操作**のため、
連打・繰り返し実行は避け、確認に必要な最小回数のみ実施すること (設定通知書シート2 E38:
検証環境DBは他事業者と共用)。

**確認手順**:

1. `/flight/dips-permission-apply` にアクセスし、「テスト申請を送信」ボタンをクリックする。
   1-9 と同じ「リンク表示 → リンクをクリック」の2段階でログイン誘導される (未連携時)
2. 成功時、「申請受付番号: Qxxxxxxxxx」の形式で受付番号が表示されることを確認する
   (ガイドライン §2.3.7 のレスポンスサンプルと同じ `Q` 始まりの形式か確認)
3. 続けて 1-9 (許可・承認情報取得) を実施し、今回送信した申請が一覧に反映されているか
   確認する (反映タイミングはガイドラインに記載がないため、即時反映されない場合は数分後に
   再確認する)
...
| 症状 | 想定原因 |
|---|---|
| 502 (DIPS連携でエラー) | DIPS 側の業務検証エラーの可能性が高い (ガイドライン §2.3.7 の 401+error配列。`lib/dips/dipsApiClient.ts` の `request()` は業務検証エラーも通信障害と同じ 502 に丸めるため、pm2 ログの `responseBody` (先頭200文字) を確認して原因コードを特定する) |
| 大阪航空局宛エラーになる | 発生しないはずだが、万一発生した場合は `lib/dips/permissionApplicationSchema.ts` の `destinationKbn`/`destinationCode` が意図せず変更されていないか確認する (検証用申請者IDの住所制約。設定通知書シート2 D16/E16) |
| その他 | 1-9 の表と同じ切り分けが適用できる (realm は `req`) |

### 1-13. 飛行計画通報受付 (5-6) の疎通確認

**前提**: realm は `fpl` (飛行計画情報取得 (1-11) と同じ Client ID)。

**背景 (req-013)**: 5-6 は 2026-07-07 から2か月間、送信項目が DIPS の必須要件 (FPRガイドライン
v1.9 §2.3.8。必須51項目) を満たさず拒否され続けていた。req-013 で不足35項目を実装し、
payload の組み立てを `lib/dips/notificationMapper.ts` に集約、必須51項目の網羅を
`__tests__/lib/dips/flightPlanNotificationPayload.test.ts` で機械的に検証している。

**事前準備 (必須)**:

1. **機体に「機体の種類」を設定する。** `/flight/aircraft/[id]/edit` で機体を編集し、
   「機体の種類」を選択して保存する。未設定のまま通報すると、DIPS へ送信する前に
   アプリ側で「機体の「機体の種類」が未設定です」のエラーになる (共用検証DBへの無駄な
   送信を防ぐガード。`DipsService.notifyFlightPlan()` 参照)。機体認証(第一種/第二種) は
   未設定 (既定 = 未取得) のままで問題ない (ガイドライン上「無し」を意味する送信になる)
2. **疎通確認専用のダミーユーザーを用意する。** 検証環境DBは他事業者と共用のため
   (設定通知書シート2 D38/E38)、実在の個人情報 (住所・電話番号) を入力しない。氏名・
   メールがダミーのユーザーでログインして通報すること

**確認手順**:

1. `/flight/plans/[id]` (承認済みかつ未通報の飛行計画) を開き、「DIPSへ通報」をクリックする
2. ダイアログの新規項目を入力する: 安全措置 (立入管理措置レベル3/3.5・立入禁止措置・
   係留飛行。該当なければチェックしない)、都道府県・住所・電話番号 (ダミー値)、技能証明
   (一等/二等/技能認証。該当なければチェックしない)。飛行目的で「その他1」「その他2」を
   選ぶと理由入力欄が表示されるので入力する
3. 「通報する」を押し、未連携なら 1-9 と同じ「リンク表示 → リンクをクリック」の2段階で
   ログイン誘導される
4. **成功時、画面に「DIPS通報済み (飛行計画ID: …)」が表示され、再通報ボタンが消えること**
   (設定通知書シート2 D38/E38「正常に登録され飛行計画IDが取得できること」)
5. 続けて 1-11 (飛行計画情報取得) を「自分の飛行計画のみ」で検索し、通報した計画が
   返ることを確認する。`flyRoute` の中心座標・半径が入力値と一致するかも確認する
   (`buildCircleFlyRoute()` の GeoJSON 形式がガイドラインのサンプルと一致することの
   事後検証。req-013 で一致を確認済みだが、実データでの再確認が望ましい)
6. **通報は必要最小回数に留める。** 失敗のたびに送り直すのではなく、次項の手順で
   拒否理由を特定してから次の1回を送る
