...
最終更新: 2026-09-08 (req-013: 飛行計画通報受付API [5-6] が必須項目不足で2か月間拒否され
続けていた問題を解消。必須51項目対応・機体マスタ拡張・通報ダイアログの入力欄追加)
...
> **2026-09-08 追記 (req-013): 「飛行計画通報のペイロード形式は暫定」は解消した。**
> 5-6 は 2026-07-07 から2か月間、**送信項目が DIPS の必須要件 (FPRガイドライン v1.9 §2.3.8。
...
> `__tests__/lib/dips/flightPlanNotificationPayload.test.ts` の REQUIRED_FIELDS 表で
> 機械的に検証する (1項目でも欠けたら テストが落ちる)。**今後この API のペイロードを
> 変更する場合、このテストの表をまずガイドラインと突合してから直すこと (推測で埋めない)。**
>
> **人の決定 (2026-09-08)**: (1) 操縦者と通報者は同一人物として扱う (ログインユーザー1人の
> 情報を両方に使う。複数名を選ぶ UI は作らない)。(2) 機体認証(第一種・第二種) は
> 貴校の全機体が未取得だが、この前提はコードにハードコードせず `Aircraft.hasDipsCertification1/2`
> (既定値 false) を読んで送信する形にした (将来取得する機体が出ても対応不要)。
>
> **未実装のまま残っているもの (許可・承認を要する飛行は通報できない)**:
> 許可・承認情報 (No.74〜87)・保険情報 (No.32〜37)・重複計画情報取得 (No.88) は
> 送信しない (いずれもガイドライン上は任意)。学校の飛行が許可・承認を要する場合、
> 現状この画面から通報できない。技能証明書番号 (No.59, 任意) は新規 PII のため送らない。
> 通報者・操縦者の住所・電話番号は都度入力とし、DB には保存しない
...
2. 上記「暫定実装」(エンドポイントパス) をガイドラインと突合・修正
3. 別紙2の固定値・別紙3のレスポンスサンプルと突合
4. 機体マスタで通報対象機体の「機体の種類」を設定する (req-013。未設定だと通報前にアプリ側で止まる)
5. 飛行計画通報 → 飛行計画情報取得の順で確認 (取得側はデータ未投入のため。手順は
   `docs/production-operations-runbook.md` の 1-13 参照)
